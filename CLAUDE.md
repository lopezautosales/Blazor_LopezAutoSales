...
- **`sass` is intentionally pinned to 1.77.x** in `Client/Styles/package.json`. Bootstrap 5.3.x isn't compatible with Dart Sass ≥1.79's new color model: `npm run build` emits 300+ deprecation warnings and compiles `$theme-color` shades to `rgb(45.88% …)` percentages instead of hex, changing the committed `wwwroot/css/bootstrap.min.css`. The CSS is checked in and neither CI nor the Dockerfile runs npm, so bumping sass buys nothing. Revisit only when Bootstrap itself supports the new color API.
